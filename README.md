...
#### Implements vibrational spectroscopic maps developed by James L. Skinner and coworkers
...
#### Additional vibrational spectroscopic map
...
Two .yml files are available. ```water-spectra.yml``` contains the environment needed to run the code using the standard electric field spectroscopic maps. ```water-spectra-delta-ML.yml``` contains the environment needed to run the code using the ∆-ML corrected spectroscopic maps [6]. The other files associated with those maps are located in the ```delta-ML-maps``` directory.
